...
Engine_TeammateGranular : CroneEngine {
  ...
  alloc {
    ...
    SynthDef('tmg_cloud', { |amp = 0.3, density = 40, grainDur = 0.12, rate = 1,
      ...
      var trig     = Dust.ar(density.max(1));
      var idx      = TWindex.ar(trig, gains);            // choisit une source (pondere)
      var posSec   = Select.ar(idx, positions);          // sa position en secondes
      var jit      = TRand.ar(jitter.neg, jitter, trig); // micro-dispersion
