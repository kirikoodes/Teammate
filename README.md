...
**MGEN — per-track length & rhythm** — each of the 16 tracks now has its own **sequence length** (a window from 1 up to 32 bars over a long master sequence) and its own **rhythmic grid against the BPM**: 1/4, 1/8, **triplets** (1/4T, 1/8T, 1/16T), 1/32 — so tracks run at different subdivisions and **phase against each other** (polyrhythm, out of straight 4/4). Editing length, grid, octave or muting a track **never re-randomizes the melody** — only *new sequence* (K3) or a style change does. A dedicated page (E3 pick track, E2 length, K2 grid) shows a **live blinking bar per active track** (you see the real rhythm) with a **selection marker** under the track you're editing and a `pas X/Y` playhead.

**NIAKABY — chord progressions** — beside the note-by-note harmonizer, NIAKABY has 4 modes (K1): **NOTE**, **FOLLOW** (deduces the chord each bar from what it hears — input/METABO/MGEN/COMP/SNOT — locked to MGEN's scale & key), **AUTO** (programmed progressions), **BLEND** (AUTO corrected by the ear). Chords are held as a pad (no per-bar re-trigger) so they don't break the groove.
...
***MGEN — longueur & rythme par piste** : chacune des 16 pistes a sa propre **longueur de séquence** (une fenêtre de 1 à 32 mesures sur une séquence maîtresse longue) et sa propre **grille rythmique face au BPM** : 1/4, 1/8, **triolets** (1/4T, 1/8T, 1/16T), 1/32 — les pistes tournent à des subdivisions différentes et **se déphasent** (polyrythmie, hors 4/4 carré). Régler la longueur, la grille, l'octave ou couper une piste **ne re-randomise jamais la mélodie** — seuls *nouvelle séquence* (K3) ou un changement de style le font. Une page dédiée (E3 choisit la piste, E2 la longueur, K2 la grille) affiche une **barre qui clignote par piste active** (on voit le rythme réel), un **repère de sélection** sous la piste éditée et un playhead `pas X/Y`. **NIAKABY — progressions d'accords** : à côté de l'harmoniseur note-à-note, 4 modes (K1) — **NOTE**, **FOLLOW** (déduit l'accord à chaque mesure de ce qu'il entend, calé sur la gamme/tonique de MGEN), **AUTO** (progressions programmées), **BLEND** (AUTO corrigé par l'oreille) ; accords tenus en nappe (pas de re-déclenchement par mesure) pour ne pas casser le groove. Au démarrage : choix **RECHERCHE** (K2, nav libre) ou **PERFORMANCE** (K3, auto-paging qui suit l'action) — mémorisé, modes ouverts restaurés ; rouvrir le menu à tout moment avec **K2 + K3**. **Molette haptique (SmartKnob, OSC)** : un bouton motorisé optionnel devient un organe de l'agent — il **tourne avec METABO** (vitesse ∝ énergie), reçoit des **à-coups sur les attaques de l'impro**, et **tourné à contre-sens** relance un nouveau thème MGEN ; auto-détectée, sinon la nav classique est inchangée. **VISION OUT** : le Norns streame son état (audio entendu, agent, METABO, MGEN) en OSC à 30 Hz vers une app externe qui s'abonne (`/teammate/subscribe`) — rien n'est envoyé sans abonné. **Horloge MIDI externe robuste** : verrouillage sur **un seul port** (anti double-comptage type OP-XY), BPM **lissé**, plage **30–300**, retour auto à l'horloge interne si les pulses s'arrêtent ; le device source est affiché sur MGEN. **CV / OSC OUT** : 8 sorties **CV / TRIG / GATE** en OSC vers un module Eurorack externe, pilotables par n'importe quelle source. **Clear corpus** : vide aussi les phrases mémorisées et coupe les grains en cours → l'agent **arrête net** de rejouer le corpus effacé.*
